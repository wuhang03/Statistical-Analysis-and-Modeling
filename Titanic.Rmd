...
ggplot(titanic_data, aes(x = factor(Survived), fill = factor(Pclass))) +
  geom_bar(position = "stack") +  # 使用position = "fill"来显示存活率
...
ggplot(titanic_data, aes(x = factor(Survived), fill = Embarked)) +
  geom_bar(position = "stack") +
  labs(x = "Survived", y = "Count") +
  ggtitle("Survival by Embarked Port (Stacked Bar Chart)")
...
```{r 建立逻辑回归模型}
# 建立逻辑回归模型
model_1 <- glm(Survived ~ Title + Pclass + Sex + Age +FamilySize + Fare + Embarked, data = train_data, family = binomial())
summary(model_1)
```
```{r}
# 模型预测和评估
...
# 建立逻辑回归模型
model <- glm(Survived ~ Title+ Pclass + Sex + Age + FamilySize , data = train_data, family = binomial())
summary(model)
```
...
```{r}
# 建立逻辑回归模型
model_1 <- glm(Survived ~ Title+ Pclass + Sex + Age_Group + FamilySize+ Fare + Embarked, data = train_data, family = binomial())
summary(model_1)
# 模型预测和评估
predictions <- predict(model_1, newdata = test_data, type = "response")
predictions <- ifelse(predictions > 0.5, 1, 0)
...
predicted_scores <- predict(model, newdata = test_data, type = "response") # 你的模型预测概率

# 调用plot_roc函数来绘制ROC曲线和计算AUC
auc_value <- plot_roc(model, actual_labels, predictions)
