...
```{r}
# 将年龄分为几个区间
titanic_data$Age_Group <- cut(titanic_data$Age, breaks = c(0, 18, 60, Inf), labels = c("Child",  "Adult", "Senior"))

# 绘制年龄组与生存情况的堆叠柱状图
ggplot(titanic_data, aes(x = factor(Survived), fill = Age_Group)) +
  geom_bar(position = "stack") +
  geom_text(stat = "count", aes(label = ..count..), position = position_stack(vjust = 0.5)) +
  labs(x = "Survived", y = "Count", fill = "Age Group") +
...
# 绘制未成年人的存活率
#senior_data <- titanic_data[titanic_data$Age_Group == 'Senior',]
#senior_survival_rate <- table(senior_data$Survived) / nrow(senior_data)
#barplot(senior_survival_rate, main = 'Senior Survival Rate', col = c('red', 'green'))
...
```{r}
# 特征重要性
importance <- summary(model_2)$coefficients
print(importance)
```
```{r}
# 建立逻辑回归模型
